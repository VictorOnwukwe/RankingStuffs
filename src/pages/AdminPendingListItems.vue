...
<script>
import ViewPendingListItem from "../components/ViewPendingListItem";
export default {
  layout: "admin",
  components: {
    "view-item": ViewPendingListItem,
  },
  data: () => ({
    page: 1,
    pageCount: 5,
  ...
  methods: {
    initialize() {
      this.fetching = true;
      this.$store
        .dispatch("fetch_pending_list_items", {
          limit: 50,
          lastDoc: false,
        })
        .then((items) => {
    ...
    async deleteItem(id) {
      try {
        await this.$store.dispatch("delete_pending_list_item", id);
        let index = this.items.findIndex((a) => a.id == id);
