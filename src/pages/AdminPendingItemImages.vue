<template>
  <v-layout>
      ...
      <template v-slot:item.action="{ item }">
        <v-menu left>
          ...
          <v-list>
            ...
            >
            <v-list-item @click="deleteItemImage(item.id)">
              <v-list-item-icon>
    ...
    <view-item
      ...
      @success="deleteItemImage(currentItem.id)"
      :update="currentItem"
    ></view-item>
...
<script>
import ViewPendingItemImage from "../components/ViewPendingItemImage";
export default {
  layout: "admin",
  components: {
    "view-item": ViewPendingItemImage
  },
  data: () => ({
    page: 1,
    pageCount: 5,
    ...
    headers: [
      ...
      { text: "Creator", value: "image.user.username" },
  ...
  methods: {
    ...
    deleteItemImage(id) {
      this.$store
        .dispatch("delete_pending_item_image", id)
        .then(() => {
