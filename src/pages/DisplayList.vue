<template>
  <div>
    <div class="main" v-if="fetched">
      <v-breadcrumbs
        class="crumbs px-0"
        :class="{ 'mt-n2': $vuetify.breakpoint.smAndDown }"
        :items="linkItems"
      >
      ...
      <v-layout class="mt-8">
        <v-flex offset-sm-1 sm10 offset-md-0 md12 lg10 offset-lg-1>
...
export default {
  ...
  head: {
    title: function() {
      return {
        inner:
          "List: " + _.startCase(this.destructureID(this.$route.params.id)),
      };
    },
  },
  ...
  methods: {
    ...
    setFavorited() {
      this.$store
        .dispatch("check_list_favorited", this.listID)
        .then((result) => {
  ...
  beforeRouteUpdate(to, from, next) {
    window.document.title =
      "List: " + _.startCase(this.destructureID(to.params.id));
    next();
  },
};
</script>
