<template>
  <div>
      ...
      <template v-slot:item.action="{ item }">
        <v-menu left>
          ...
          <v-list>
            ...
            >
            <v-list-item @click="deleteList(item.pend_id)">
              <v-list-item-icon>
    ...
    <view-list
      ...
      @success="deleteList(currentList.id)"
      :list="currentList"
    ></view-list>
...
<script>
import ViewPendingList from "../components/ViewPendingList";
export default {
  layout: "admin",
  components: {
  ...
  methods: {
    ...
    async deleteList(id) {
      try {
        await this.$store.dispatch("delete_pending_list", id);
        let index = this.lists.findIndex((a) => a.pend_id == id);
