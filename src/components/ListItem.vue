    ...
    <v-card class="mt-2" outlined>
      <v-card-title class="px-2 py-2 top-bar">
        <v-layout align-center>
          ...
          <v-flex shrink class>
            <div v-if="!voted">
              <v-icon @click="vote()" class="action-icon" size="1.3em">fa-vote-yea</v-icon>
            </div>
            <v-layout v-else>
              ...
              <v-layout class style="width:2.4375em; border-radius:.3em" justify-center>
                <h4 style="font-weight:bold;display:inline-block">{{votePercentage}}%</h4>
              </v-layout>
            </v-layout>
          </v-flex>
        </v-layout>
      ...
      <v-layout wrap>
        <v-flex xs12 sm4 xl3>
          <v-card tile class="pr-0" flat height="100%">
  ...
  methods: {
    ...
    vote() {
      if (!this.authenticated) {
        this.showAuthenticationError("vote");
        return;
      }
      this.$emit("voted");
