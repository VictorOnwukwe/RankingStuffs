  ...
  <v-layout class="py-4 px-2 mt-4">
    <v-flex xs12>
      <v-card flat tile class="mb-4">
        <v-card-title class="text-capitalize top-bar pa-1 pl-2 title-text font-weight-medium">Items</v-card-title>
        <v-card-text class="pa-0">
          ...
          <v-list v-if="favoriteItems.length > 0">
            ...
            >
              ...
              <v-list-item-content>
                <v-list-item-title class="text-capitalize link--text text-wrap">{{favItem.data().name}}</v-list-item-title>
                <v-list-item-subtitle class="subtitle-2">{{favItem.id}}</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
          </v-list>
          <p v-else class="subtitle-1 mt-4 std text-center">No Favorite Items...</p>
          <v-layout class="my-4" v-if="fetchingMoreItems" justify-center>
            <m-progress></m-progress>
          </v-layout>
          <v-layout v-if="favoriteItems.length !== user.favorite_items && favoriteItems.length > 0" class="mt-4" justify-center>
            <v-icon @click="fetchMoreItems()" size="32">mdi-plus-circle-outline</v-icon>
          </v-layout>
          ...
          >
            <v-hover v-slot:default="{ hover }">
              <v-layout
                :class="{'grey lighten-2': addFavorites, 'grey lighten-4': hover}"
                pl-2
                align-center
                py-1
              ...
              >
                <v-icon
                  :color="addFavorites ? 'accent' : 'grey lighten-1'"
                  size="2.5em"
                >mdi-plus-box</v-icon>
                <div
                  class="ml-2"
                  style="font-weight:normal"
                  :class="addFavorites ? 'grey--text text--darken-3 font-weight-bold' : 'grey--text'"
              ...
              <v-card class="grey lighten-3" tile flat>
                ...
                <v-card-actions>
                  <v-btn
                    ...
                    @click="addFavorite()"
                    class="brand darken-1 ml-n4 white--text"
                  >Add</v-btn>
                </v-card-actions>
              </v-card>
            </div>
      ...
      <v-card tile flat class>
        ...
        <v-card-text class="pa-0">
          ...
          <div v-else-if="favoriteLists.length > 0">
            <v-list>
              <template v-for="(list, index) in favoriteLists">
                <list-preview :id="list.data().list" :key="list.id"></list-preview>
                <v-divider class="grey lighten-3" style="margin-left:100px" v-if="index < favoriteLists.length - 1" :key="index"></v-divider>
              </template>
            </v-list>
          ...
          <div v-else class="subtitle-1 mt-4 std text-center">No Favorite Lists...</div>
        </v-card-text>
      </v-card>
    </v-flex>
  </v-layout>
  ...
  methods: {
    ...
    fetchMoreItems() {
      ...
      this.$store
        .dispatch("fetch_favorite_items", {
          limit: 10,
          user: this.user.id,
          timestamp: this.favoriteItems[this.favoriteItems.length - 1].data().created
        })
        .then(results => {
    ...
    fetchMoreLists() {
      ...
      this.$store
        .dispatch("fetch_favorite_lists", {
          limit: 10,
          timestamp: this.favoriteLists[this.favoriteLists.length - 1].data().created,
          user: this.user.id
        })
        .then(results => {
          this.favoriteLists = this.favoriteLists.concat(results);
